...
>
	{#if prismic.isFilled.image(slice.primary.backgroundImage)}
		<PrismicImage
			...
			alt=""
			class="absolute inset-0 h-full w-full pointer-events-none select-none object-cover opacity-20"
		/>
	{/if}
		...
		<div class="grid justify-items-center gap-8">
			...
			{#if prismic.isFilled.link(slice.primary.buttonLink)}
				<PrismicLink
					...
					class="rounded text-teal-50 px-5 py-3 font-medium bg-slate-900"
				>
